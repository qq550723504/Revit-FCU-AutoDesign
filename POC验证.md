...
## 2026-09-16 FCU 房间中心布置修复

用户现场复测显示：首房间可连接，后续房间靠近门侧布置时冷凝水候选受墙体和供水下翻限制；用户确认把 FCU 放在房间中部可避免该问题。检查代码确认旧规则实际为“门点沿房间内缩 800 mm”，不是房间中心。该定位规则会把设备和首段管线固定在门侧附近，导致可行空间被门墙和既有管线占用。

现改为默认“优先布置在房间几何中心”：门及门墙只用于确定 L 轴和 FCU 出风/接口朝向；落点由房间边界在 L/H 两个局部轴上的中点计算，并通过 `Room.IsPointInRoom` 校验。界面取消中心选项后才使用原门内缩距离。中心点计算失败时不会伪造坐标，仍走原有房间内校验并报告失败。

PASS：Revit 2020/.NET 4.8 Release Rebuild Exit 0；`Verify-Dialog.ps1 -AssemblyPath .\bin\Release\FCUAutoDesign.dll` 输出 `47 checks passed`，覆盖中心默认值、门侧覆盖、恢复中心和参数读取。真实房间边界、族包络、家具障碍、墙体穿越和多房间接管为 NOT_RUN。
