...
冷凝水错层绕行修复：原有 25 条候选失败后，追加 30 条水平出管、竖直错层、横移及主管接入候选。新增候选采用 150/250/400/600/800 mm 出管长度及相对接口 ±200/400/600 mm 的横移高度（DN20）。不设置坡度或限制高差方向，仍要求完整连接及实体干涉检查通过。几何测试覆盖水平出管、正交弯头、实际主管标高和短管段拒绝；真实 Revit 管件生成及本例碰撞消除仍需模型验证。
