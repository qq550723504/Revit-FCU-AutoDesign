...
## 2026-09-16 FCU 门侧墙 500 mm 布置规则

用户确认 FCU 不是完全放在房间几何中心，而是距门所在墙室内侧 500 mm，沿墙方向取房间中部。检查代码确认旧规则实际为“门点沿房间内缩 800 mm”，不是墙距规则。该定位规则会随门位置变化，造成同一门墙上不同门位得到不同设备落点。

现改为默认“距门侧墙内侧 500 mm、沿墙方向居中”：门及门墙定义基准墙和朝向，墙厚半值加 500 mm 得到 FCU 中心落点，沿墙方向取房间边界中点，并通过 `Room.IsPointInRoom` 校验。界面勾选几何中心后才使用中心规则；不再使用门点到设备点的 800 mm 假设。目标点超出房间时不伪造坐标，仍报告定位失败。

PASS：Revit 2020/.NET 4.8 Release Rebuild Exit 0；界面回归覆盖 500 mm 默认值、墙侧规则开关、几何中心覆盖和参数读取。真实墙厚、房间边界、族包络、家具障碍、墙体穿越和多房间接管为 NOT_RUN。
