...
namespace FCUAutoDesign
{
    internal class CondensateSeparationService
    {
        ...
        public CondensateDrainResult Connect(Document doc, Connector connector, Pipe main,
            ...
            TeeConnectionResult supply, TeeConnectionResult ret, MainPipeRun run = null, RoomBatchContext batch = null)
        {
            ...
            int attempts = 0;
            foreach (bool orthogonal in new[] { false, true })
            {
                int[] offsets = orthogonal ? new[] { -2, 2, -4, 4, -6, 6 } : new[] { 0, -1, 1, -2, 2 };
                    ...
                    foreach (int offset in offsets)
                    {
                        ...
                        using (SubTransaction candidate = new SubTransaction(doc))
                        {
                            ...
                            try
                            {
                                ...
                                double lead = orthogonal
                                    ? new[] { 150, 250, 400, 600, 800 }[leadStep] * MM_TO_FEET
                                    : 400 * MM_TO_FEET + leadStep * step;
                                CondensateDrainResult result = drain.ConnectToMain(doc, current,
                                    doc.GetElement(mainId) as Pipe, levelId, diameter, reporter, lead, offset * step, run, orthogonal);
                                if (!result.Connected) throw new InvalidOperationException(result.ErrorMessage);
                                doc.Regenerate();
                                Verify(doc, result, supply, ret);
                                batch?.VerifyNew(doc, result.Connection);
                                if (candidate.Commit() != TransactionStatus.Committed)
                                    throw new InvalidOperationException("冷凝水避让候选未成功提交。");
                                if (orthogonal)
                                    result.ErrorMessage = $"为避开供回水，冷凝水水平出管 {lead * FEET_TO_MM:F0} mm 后竖直错层，"
                                        + $"横移标高相对设备接口偏移 {offset * step * FEET_TO_MM:F0} mm，再接入主管。";
                                else if (leadStep != 0 || offset != 0)
            ...
            {
                ErrorMessage = $"{attempts} 条冷凝水候选均未通过接管与供回水干涉检查；未保留本次冷凝水操作。最后原因：" + lastReason
            };
        }
